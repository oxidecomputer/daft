...
Daft consists of a trait called [`Diffable`](https://docs.rs/daft/0.1.4/daft/diffable/trait.Diffable.html), along with [a derive
macro](https://docs.rs/daft-derive/0.1.4/daft_derive/derive.Diffable.html) by the same name. This trait represents the
...
#### [`Leaf`](https://docs.rs/daft/0.1.4/daft/leaf/struct.Leaf.html) instances

A [`Leaf`](https://docs.rs/daft/0.1.4/daft/leaf/struct.Leaf.html) represents a logical *leaf node* or *base case* in a diff, i.e. a
point at which diffing stops. [`Leaf`](https://docs.rs/daft/0.1.4/daft/leaf/struct.Leaf.html) instances are used for:
...
For [`BTreeMap`] and [`HashMap`], daft has corresponding [`BTreeMapDiff`](https://docs.rs/daft/0.1.4/daft/alloc_impls/struct.BTreeMapDiff.html)
and [`HashMapDiff`](https://docs.rs/daft/0.1.4/daft/std_impls/struct.HashMapDiff.html) types. These types have fields for *common*, *added*,
and *removed* entries.
...
For [`BTreeSet`] and [`HashSet`], daft has corresponding [`BTreeSetDiff`](https://docs.rs/daft/0.1.4/daft/alloc_impls/struct.BTreeSetDiff.html)
and [`HashSetDiff`](https://docs.rs/daft/0.1.4/daft/std_impls/struct.HashSetDiff.html) types. These types have fields for *common*, *added*,
and *removed* entries.
...
For a tuple like `(A, B, C)`, the [`Diffable`](https://docs.rs/daft/0.1.4/daft/diffable/trait.Diffable.html) implementation is recursive:
...
For structs, the [`Diffable`](https://docs.rs/daft-derive/0.1.4/daft_derive/derive.Diffable.html) derive macro generates
a diff type with a field corresponding to each field type. Each field must
implement [`Diffable`](https://docs.rs/daft/0.1.4/daft/diffable/trait.Diffable.html).
...
* `#[daft(leaf)]`: if a **struct** is annotated with this, the [`Diffable`](https://docs.rs/daft/0.1.4/daft/diffable/trait.Diffable.html)
  implementation for the struct will be a [`Leaf`](https://docs.rs/daft/0.1.4/daft/leaf/struct.Leaf.html) instead of a recursive
  diff.

##### Field options

* `#[daft(leaf)]`: if a  **struct field** is annotated with this, the generated
  struct’s corresponding field will be a [`Leaf`](https://docs.rs/daft/0.1.4/daft/leaf/struct.Leaf.html), regardless of the field’s
  `Diff` type (or even whether it implements [`Diffable`](https://docs.rs/daft/0.1.4/daft/diffable/trait.Diffable.html) at all).
...
The [`Diffable`](https://docs.rs/daft/0.1.4/daft/diffable/trait.Diffable.html) trait can also be implemented manually for custom behavior.
...
If a type parameter is specified, the [`Diffable`](https://docs.rs/daft-derive/0.1.4/daft_derive/derive.Diffable.html) derive
...
* Daft has the notion of a [`Leaf`](https://docs.rs/daft/0.1.4/daft/leaf/struct.Leaf.html) type, which represents an atomic unit.
  (For example, the [`Diffable`](https://docs.rs/daft/0.1.4/daft/diffable/trait.Diffable.html) implementation for `i32` is a [`Leaf`](https://docs.rs/daft/0.1.4/daft/leaf/struct.Leaf.html).)
  [`Leaf`](https://docs.rs/daft/0.1.4/daft/leaf/struct.Leaf.html)s are also used for enums, as well as in any other place where lazy
  diffing is desired.
